...
// Update handles input for the pause menu. Navigation (keyboard arrows,
// gamepad D-pad, and left analog stick) and the activate/back/start actions
// are supplied by the shared InputManager via nav, keeping behavior identical
// to the rest of the UI. Mouse and keyboard Enter/Space are handled directly.
func (m *PauseMenu) Update(nav UINavigation) {
	if !m.visible {
		return
	}

	// Navigation and activate/back actions from the shared InputManager.
	if m.applyNavigation(nav) {
		return
	}

	// Keyboard selection (Enter/Space is not part of UINavigation)
...
// applyNavigation applies shared InputManager navigation to the menu. Up/Down
// move the selection (clamped, no wrap); Back (ESC/B) and Start resume; Activate
// (gamepad A) selects. Returns true when the menu was resolved (resumed or a
// selection was made), signalling that no further input should be processed.
func (m *PauseMenu) applyNavigation(nav UINavigation) bool {
	...
	// Back (ESC / B) or Start acts as Resume
	...
	// Activate (gamepad A) selects the current option
	if nav.Activate {
		m.handleSelect()
		return true
	}
